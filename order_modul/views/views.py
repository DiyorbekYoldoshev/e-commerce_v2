...
from order_modul.serializers import (
    ...
    OrderItemSerializer, OrderItemCreateSerializer, CouponSerializer
)
...
class OrderViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        # items are expected in payload as a list of {product:id, quantity:int}
        ...
        # ensure 'items' is not passed into create_order via validated data
        validated_order_data = dict(serializer.validated_data)
        validated_order_data.pop('items', None)

        try:
            order = create_order(request.user, validated_order_data, hydrated_items)
    ...
    # Admin / manager actions to change status
